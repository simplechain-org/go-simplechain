...
func NewTxPool(config *params.ChainConfig, chain *LightChain, relay TxRelayBackend) *TxPool {
	pool := &TxPool{
		config:      config,
		signer:      types.NewEIP155Signer(config.GetChainID(chain.CurrentHeader().Number)),
...
func (pool *TxPool) setNewHead(head *types.Header) {
	...
	pool.singularity = pool.config.IsSingularity(next)
}
...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	gas, err := core.IntrinsicGas(tx.Data(), tx.To() == nil, pool.singularity)
	if err != nil {
		return err
	}
	if tx.Gas() < gas {
