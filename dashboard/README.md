## Go Simplechain Dashboard

The dashboard is a data visualizer integrated into sipe, intended to collect and visualize useful information of an Simplechain node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-simplechain` repo), these need to be installed first:

```
$ (cd dashboard/assets && yarn install && yarn flow)
```

Normally the dashboard assets are bundled into Sipe via `go-bindata` to avoid external dependencies. Rebuilding Sipe after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ sipe --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into Sipe, run `go generate`:
