...
// with pieces plucked from go-simplechain, rather to allow writing native dapps on
...
// from the go-simplechain repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-simplechain version.
