...
func TestQueue(t *testing.T) {
	...
	for i, tt := range tests {
		...
		snap := &Snapshot{
			config:   &params.DPoSConfig{MaxSignerCount: tt.maxSignerCount},
			Number:   tt.number,
			LCRS:     1,
		...
		snap.Hash = common.HexToHash(tt.historyHash[len(tt.historyHash)-1])
		for _, hash := range tt.historyHash {
			hh := common.HexToHash(hash)
