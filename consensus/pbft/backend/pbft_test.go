...
func getGenesisAndKeys(n int) (*core.Genesis, []*ecdsa.PrivateKey) {
	...
	genesis.Mixhash = types.PbftDigest
...
func TestSealCommittedOtherHash(t *testing.T) {
	...
	go func() {
		ev := <-eventSub.Chan()
		if _, ok := ev.Data.(pbft.RequestEvent); !ok {
			t.Errorf("unexpected event comes: %v", reflect.TypeOf(ev.Data))
		}
...
func TestSealCommitted(t *testing.T) {
	chain, engine := newBlockChain(1)
	block := makeBlockWithoutSeal(chain, engine, chain.Genesis())
	expectedBlock, _ := engine.signBlock(engine.chain.GetHeader(block.ParentHash(), block.NumberU64()-1), block)
	resultCh := make(chan *types.Block, 10)
	go func() {
...
func TestVerifyHeaders(t *testing.T) {
	...
	for i := 0; i < size; i++ {
		...
		if i == 0 {
			...
			b, _ = engine.signBlock(genesis.Header(), b)
		} else {
			b = makeBlockWithoutSeal(chain, engine, blocks[i-1])
			b, _ = engine.signBlock(blocks[i-1].Header(), b)
		}
...
func TestWriteSeal(t *testing.T) {
	...
	expectedIstExtra := &types.ByzantineExtra{
	...
	istExtra, err := types.ExtractByzantineExtra(h)
	if err != nil {
		t.Errorf("error mismatch: have %v, want nil", err)
	}
