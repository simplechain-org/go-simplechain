...
type defaultSet struct {
	validators pbft.Validators
	policy     pbft.ProposerPolicy

	proposer    pbft.Validator
	validatorMu sync.RWMutex
	selector    pbft.ProposalSelector
}

func newDefaultSet(addrs []common.Address, policy pbft.ProposerPolicy) *defaultSet {
	valSet := &defaultSet{}
...
func (valSet *defaultSet) List() []pbft.Validator {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
...
func calcSeed(valSet pbft.ValidatorSet, proposer common.Address, round uint64) uint64 {
	offset := 0
...
func roundRobinProposer(valSet pbft.ValidatorSet, proposer common.Address, round uint64) pbft.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func stickyProposer(valSet pbft.ValidatorSet, proposer common.Address, round uint64) pbft.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func (valSet *defaultSet) Copy() pbft.ValidatorSet {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
...
func (valSet *defaultSet) Policy() pbft.ProposerPolicy { return valSet.policy }
