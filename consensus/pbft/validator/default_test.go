...
func testNewValidatorSet(t *testing.T) {
	...
	// Create ValidatorSet
	valSet := NewSet(ExtractValidators(b), pbft.RoundRobin)
	if valSet == nil {
...
func testNormalValSet(t *testing.T) {
	...
	valSet := newDefaultSet([]common.Address{addr1, addr2}, pbft.RoundRobin)
	if valSet == nil {
...
func testEmptyValSet(t *testing.T) {
	valSet := NewSet(ExtractValidators([]byte{}), pbft.RoundRobin)
	if valSet == nil {
...
func testAddAndRemoveValidator(t *testing.T) {
	valSet := NewSet(ExtractValidators([]byte{}), pbft.RoundRobin)
	if !valSet.AddValidator(common.BytesToAddress([]byte(string(rune(2))))) {
		t.Error("the validator should be added")
	}
	if valSet.AddValidator(common.BytesToAddress([]byte(string(rune(2))))) {
		t.Error("the existing validator should not be added")
	}
	valSet.AddValidator(common.BytesToAddress([]byte(string(rune(1)))))
	valSet.AddValidator(common.BytesToAddress([]byte(string(rune(0)))))
	if len(valSet.List()) != 3 {
		t.Error("the size of validator set should be 3")
	}

	for i, v := range valSet.List() {
		expected := common.BytesToAddress([]byte(string(rune(i))))
		if v.Address() != expected {
			t.Errorf("the order of validators is wrong: have %v, want %v", v.Address().Hex(), expected.Hex())
		}
	}

	if !valSet.RemoveValidator(common.BytesToAddress([]byte(string(rune(2))))) {
	...
	valSet.RemoveValidator(common.BytesToAddress([]byte(string(rune(1)))))
	if len(valSet.List()) != 1 {
		t.Error("the size of validator set should be 1")
	}
	valSet.RemoveValidator(common.BytesToAddress([]byte(string(rune(0)))))
...
func testStickyProposer(t *testing.T) {
	...
	valSet := newDefaultSet([]common.Address{addr1, addr2}, pbft.Sticky)
