...
OUTER:
	for _, test := range testCases {
		...
		if r0.state != StateCommitted {
			...
			if r0.current.Commits.Size() >= r0.Confirmations() {
				t.Errorf("the size of commit messages should be less than %v", r0.Confirmations())
			}
			if r0.current.IsHashLocked() {
				t.Errorf("block should not be locked")
			}
		...
		if r0.current.Commits.Size() < r0.Confirmations() {
			t.Errorf("the size of commit messages should be larger than 2F+1 or Ceil(2N/3): size %v", r0.Confirmations())
		}
