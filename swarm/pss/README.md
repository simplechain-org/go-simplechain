...
`pss` is under active development, and the first implementation is yet to be merged to the Simplechain main branch. Expect things to change.
...
The code tutorial [p2p programming in go-simplechain](https://github.com/nolash/go-simplechain-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-simplechain` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-simplechain` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
