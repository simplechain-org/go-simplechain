...
#
# The cluster contains a bootnode, a sipe node and multiple swarm nodes, with
...
# static sipe configuration
SIPE_IP="192.168.33.3"
SIPE_RPC_PORT="8545"
SIPE_RPC_URL="http://${SIPE_IP}:${SIPE_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the sipe and
# swarm nodes
start_bootnode() {
...
# start_sipe_node starts a sipe node with --datadir pointing at <base-dir>/sipe
# and a single, unlocked account with password "sipe"
start_sipe_node() {
  local dir="${base_dir}/sipe"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get sipe account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${SIPE_IP}"
    --rpcport   "${SIPE_RPC_PORT}"
    --verbosity "6"
  )

  start_node "sipe" "${SIPE_IP}" "$(which sipe)" ${args[@]}
}
...
create_account() {
  ...
  sipe --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
