...
func (pm *ProtocolManager) syncCtxs(p *peer) {
	...
	remote,local := pm.msgHandler.GetCtxstore().Query()

	var rtxs,ltxs []*types.CrossTransactionWithSignatures
	...
	if len(remote) + len(local) == 0 {
		return
	}
	log.Info("syncCtxs", "remote", len(rtxs),"local",len(ltxs))
	go func() {
		select {
		case pm.ctxsyncCh <- &ctxsync{p, rtxs}:
			log.Info("ctxsyncCh")
		case <-pm.quitSync:
		}
	}()
	go func() {
		select {
		case pm.ctxsyncLocal <- &ctxsync{p,ltxs}:
			log.Info("ctxsyncLocal")
		case <-pm.quitSync:
		}
	}()
}
...
func (pm *ProtocolManager) ctxlocalsyncLoop() {
	var (
		pending = make(map[enode.ID]*ctxsync)
		sending = false               // whether a send is active
		pack    = new(ctxsync)        // the pack that is being sent
	...
	send := func(s *ctxsync) {
		...
		// Send the pack in the background.
		s.p.Log().Info("Sending batch of InternalCrossTransactionWithSignatures", "count", len(pack.txs), "bytes", size)
		sending = true
		go func() { done <- pack.p.SendInternalCrossTransactionWithSignatures(pack.txs) }()
	}
	...
	for {
		...
		case err := <-done:
			...
			// Stop tracking peers that cause send failures.
			if err != nil {
				pack.p.Log().Info("InternalCrossTransactionWithSignatures send failed", "err", err)
				delete(pending, pack.p.ID())
			}
			// Schedule the next send.
			if s := pick(); s != nil {
