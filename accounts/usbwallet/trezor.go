...
import (
	...
	"github.com/simplechain-org/go-simplechain/accounts/usbwallet/internal/trezor"
...
// or whether the Simplechain app was not started on it.
...
// Simplechain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.SimplechainAddress)
	if _, err := w.trezorExchange(&trezor.SimplechainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.SimplechainSignTx{
	...
	response := new(trezor.SimplechainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.SimplechainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
