...
import (
	...
	Simplechain "github.com/simplechain-org/go-simplechain"
...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Simplechain
...
type wallet struct {
	...
	deriveChain    Simplechain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Simplechain account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain Simplechain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
