...
type CrossStore struct {
	...
	anchors     map[uint64]*core.AnchorSet
...
func NewCrossStore(ctx crossdb.ServiceContext, config cross.CtxStoreConfig, chainConfig *params.ChainConfig, chain *core.BlockChain,
	makerDb string, address common.Address, signHash types.SignHash) (*CrossStore, error) {
	...
	store := &CrossStore{
		...
		signer:        signer,
		anchors:       make(map[uint64]*core.AnchorSet),
		mu:            sync.RWMutex{},
...
func (store *CrossStore) AddLocal(ctx *types.CrossTransaction) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	return store.addTx(ctx, true)
}

func (store *CrossStore) AddRemote(ctx *types.CrossTransaction) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *CrossStore) AddFromRemoteChain(ctx *types.CrossTransactionWithSignatures, callback func(*types.CrossTransactionWithSignatures, ...int)) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *CrossStore) addTx(ctx *types.CrossTransaction, local bool) error {
...
func (store *CrossStore) addTxLocked(ctx *types.CrossTransaction, local bool) error {
	id := ctx.ID()
	...
	checkAndCommit := func(id common.Hash) error {
		if cws := store.pending.Get(id); cws != nil && cws.SignaturesLength() >= requireSignatureCount {
			...
			go store.resultFeed.Send(core.SignedCtxEvent{
				Tws: cws.Copy(),
	...
	if cws := store.queued.Get(id); cws != nil {
		...
		store.queued.Put(types.NewCrossTransactionWithSignatures(ctx), core.NewChainInvoke(store.chain).GetTransactionNumberOnChain(ctx))
	}
	return nil
}

func (store *CrossStore) VerifyCtx(ctx *types.CrossTransaction) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	...
	// discard if expired
	if core.NewChainInvoke(store.chain).IsTransactionInExpiredBlock(ctx, expireNumber) {
...
func (store *CrossStore) verifySigner(ctx *types.CrossTransaction, signChain, destChain *big.Int) error {
	store.logger.Debug("verify ctx signer", "ctx", ctx.ID(), "signChain", signChain, "destChain", destChain)
	var anchorSet *core.AnchorSet
	if as, ok := store.anchors[destChain.Uint64()]; ok {
		...
		anchors, signedCount := core.QueryAnchor(store.chainConfig, store.chain, statedb, newHead, store.CrossContract, destChain.Uint64())
		store.config.Anchors = anchors
		requireSignatureCount = signedCount
		anchorSet = core.NewAnchorSet(store.config.Anchors)
...
func (store *CrossStore) verifyCwsInvoking(cws core.CrossTransactionInvoke) error {
	...
	evmInvoke := core.NewEvmInvoke(store.chain, store.chain.CurrentBlock().Header(), stateDB, config, vm.Config{})
	var res []byte
...
func (store *CrossStore) RemoveRemotes(rtxs []*types.ReceptTransaction) []error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *CrossStore) MarkStatus(rtxs []*types.ReceptTransaction, status types.CtxStatus) {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *CrossStore) Query() (map[uint64][]*types.CrossTransactionWithSignatures, map[uint64][]*types.CrossTransactionWithSignatures) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *CrossStore) GetSyncCrossTransactions(chainID uint64, txID common.Hash, pageSize int) []*types.CrossTransactionWithSignatures {
...
func (store *CrossStore) ListCrossTransactionBySender(from common.Address) (map[uint64][]*types.CrossTransactionWithSignatures, map[uint64][]*types.CrossTransactionWithSignatures) {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *CrossStore) SubscribeSignedCtxEvent(ch chan<- core.SignedCtxEvent) event.Subscription {
	store.mu.Lock()
	defer store.mu.Unlock()
	return store.resultScope.Track(store.resultFeed.Subscribe(ch))
}

func (store *CrossStore) UpdateAnchors(info *types.RemoteChainInfo) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	newHead := store.chain.CurrentBlock().Header() // Special case during testing
	statedb, err := store.chain.StateAt(newHead.Root)
	...
	anchors, signedCount := core.QueryAnchor(store.chainConfig, store.chain, statedb, newHead, store.CrossContract, info.RemoteChainId)
	store.config.Anchors = anchors
	requireSignatureCount = signedCount
	store.anchors[info.RemoteChainId] = core.NewAnchorSet(store.config.Anchors)
	return nil
}
...
func (store *CrossStore) SyncCrossTransactions(ctxList []*types.CrossTransactionWithSignatures) int {
	store.mu.Lock()
	defer store.mu.Unlock()
