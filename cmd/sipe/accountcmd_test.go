...
func TestAccountListEmpty(t *testing.T) {
	sipe := runSipe(t, "account", "list")
	sipe.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	sipe := runSipe(t, "account", "list", "--datadir", datadir)
	defer sipe.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	sipe := runSipe(t, "account", "new", "--lightkdf")
	defer sipe.ExpectExit()
	sipe.Expect(`
...
`)
	sipe.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	sipe := runSipe(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer sipe.ExpectExit()
	sipe.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(sipe.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(sipe.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	sipe.SetTemplateFunc("keypath", func(file string) string {
