...
// nodeDockerfile is the Dockerfile required to run an Simplechain node.
var nodeDockerfile = `
FROM simplechain/client-go:latest
...
{{end}}
RUN \
  echo 'sipe --cache 512 init /genesis.json' > sipe.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.simplechain/keystore/ && cp /signer.json /root/.simplechain/keystore/' >> sipe.sh && \{{end}}
	echo $'sipe --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> sipe.sh

ENTRYPOINT ["/bin/sh", "sipe.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Simplechain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.simplechain{{if .Ethashdir}}
...
// deployNode deploys a new Simplechain node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 sipe --exec admin.nodeInfo.id --cache=16 attach", network, kind)); err != nil {
