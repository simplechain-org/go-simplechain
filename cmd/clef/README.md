...
This setup allows a DApp to connect to a remote Simplechain node and send transactions that are locally signed. This can
help in situations when a DApp is connected to a remote node because a local Simplechain node is not available, not
synchronised with the chain or a particular Simplechain node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.simplechain/keystore")
...
The general flow for signing a transaction using e.g. sipe is as follows:
![image](sign_flow.png)

In this case, `sipe` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
The signer will generate a new private key, encrypts it and stores it in the keystore directory.
...
#### Arguments
  - account [object]: retrieved with account_export 
...
* A Graphical UI MUST show the blocky-identicon for simplechain addresses.
