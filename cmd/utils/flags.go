...
var (
	...
	//todo set localhost:8801 for default
...
// RegisterEthService adds an Simplechain client to the stack.
...
// RegisterEthStatsService configures the Simplechain Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Simplechain
		ctx.Service(&ethServ)

		var lesServ *les.LightSimplechain
		ctx.Service(&lesServ)

		return ethstats.New(url, ethServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Simplechain Stats service: %v", err)
	}
}
...
// new format.
//
// e.g. sipe account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// sipe --keystore /tmp/mykeystore --lightkdf account new
//
